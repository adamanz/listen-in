#!/usr/bin/env python3
"""Test pipeline using o3 model via agents API."""
...
from listen_in.generators.agent_generator import AgentGenerator
...
async def test_o3_pipeline():
    """Test the pipeline using o3 model."""
    
    print("🎙️ Listen-in Agent SDK Pipeline Test")
    print("=" * 50)
    ...
    # Set up paths - use the GDPR excerpt file
    input_file = "/Users/adamanzuoni/listen-in/examples/gdpr_excerpt_test.txt"
    output_dir = Path.home() / "Desktop" / "listen-in-output-o3"
    ...
    # Step 2: Generate podcast script with Agent SDK
    print("\n2️⃣ Generating podcast script with gpt-4.1-mini via Agent SDK...")
    print("   Note: This uses structured output and may take longer...")
    try:
        generator = AgentGenerator(api_key=openai_key)
        ...
        script_filename = f"gdpr_podcast_o3_{timestamp}.md"
        script_path = output_dir / script_filename
        save_script(script, str(script_path))
        
        print(f"✅ Script generated with o3: {script_path}")
    ...
    try:
        ...
        audio_filename = f"gdpr_podcast_o3_{timestamp}.mp3"
...
async def main():
    ...
    audio_path = await test_o3_pipeline()
