...
const PAYMENT_RECIPIENT = process.env.PAYMENT_RECIPIENT as `0x${string}`;
...
async function processPayment(toolName: string): Promise<{ success: boolean; txHash?: string; error?: string }> {
  try {
...
async function callElevenLabsServer(path: string, params: any): Promise<any> {
  try {
...
server.tool(
  "list_voices",
  "List all available ElevenLabs voices with their details. Costs $0.10.",
  {},
  async () => {
...
server.tool(
  "text_to_speech",
  "Convert text to speech using ElevenLabs API and save it to a file. Costs $0.10.",
  {
    text: z.string(),
...
server.tool(
  "get_voice_settings",
  "Get the default voice settings for a specific voice. Costs $0.10.",
  {
    voice_id: z.string(),
  },
  async (args) => {
...
server.tool(
  "check_api_status",
  "Check if the ElevenLabs API is configured and accessible. Costs $0.10.",
  {},
  async () => {
    // Process payment first
...
server.tool(
  "get_payment_history",
  "Get the payment history for this session. This tool is free.",
  {},
  async () => {
    return {
      content: [{
