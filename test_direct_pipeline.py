#!/usr/bin/env python3
"""Test pipeline using direct component access instead of MCP tools."""
...
async def test_direct_pipeline():
    """Test the pipeline by directly using components."""
    
    print("🎙️ Listen-in Direct Pipeline Test")
    print("=" * 50)
    ...
    try:
        ...
        script_filename = f"gdpr_podcast_{timestamp}.md"
    ...
    try:
        audio_generator = AudioGenerator(api_key=elevenlabs_key)
        ...
        audio_filename = f"gdpr_podcast_{timestamp}.mp3"
        ...
        if result.get("status") == "completed":
            ...
            print(f"   Duration scale: {result['duration_scale']}")
...
async def main():
    ...
    audio_path = await test_direct_pipeline()
